...
namespace SqliteWasmHelper
{
    ...
    public sealed class BrowserCache : IAsyncDisposable, IBrowserCache
    {
        ...
        /// <summary>
        /// Loads static database file into cache.
        /// </summary>
        /// <returns>Nothing.</returns>
        public async Task LoadDatabaseFromFile()
        {
            var stream = await new HttpClient().GetStreamAsync("http://localhost:5120/sample-data/chinook.db"); // file in the wwwroot
            ...
            var module = await moduleTask.Value;
            await module.InvokeVoidAsync("writeToCache", base64);
